...
export default function MenuCard({ item, index }: MenuCardProps) {
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-xl sm:rounded-2xl overflow-hidden flex-shrink-0">
        <img
          src={item.image}
          alt={item.name}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
        ...
        <div className="flex items-center justify-between mt-2 sm:mt-3">
          <span
            className="text-lg sm:text-xl font-bold bg-gradient-to-r from-orange-400 to-amber-500 bg-clip-text text-transparent"
          ...
          {quantity === 0 ? (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={handleAdd}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl bg-gradient-to-r from-orange-500 to-amber-600 text-white text-xs sm:text-sm font-semibold shadow-md shadow-orange-500/20 hover:shadow-orange-500/40 transition-shadow"
            >
              <Plus className="w-3.5 h-3.5" />
              Add
            </motion.button>
          ) : (
            <div className="flex items-center gap-2 sm:gap-3 bg-gradient-to-r from-orange-500/10 to-amber-500/10 border border-orange-500/20 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5">
              <motion.button
                whileTap={{ scale: 0.85 }}
              ...
              <span className="text-sm sm:text-base font-bold text-orange-500 min-w-[16px] text-center">
                {quantity}
              </span>
              <motion.button
                whileTap={{ scale: 0.85 }}
